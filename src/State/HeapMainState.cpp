...
void HeapMainState::initNode()
{
	for (int i = 0; i < maxNodes; i++)
	{
		...
		if (i == 0) { // the root
			...
			if (i % 2 == 1) { // left child
				node.setPosition(sf::Vector2f(b_nodes[par].getPosition().x - std::max(minSpaceX, static_cast<float>(spaceX / pow(2, dep - 1))), b_nodes[par].getPosition().y + 2 * RADIUS + spaceY - dep));
			}
			else { // right child
				node.setPosition(sf::Vector2f(b_nodes[par].getPosition().x + std::max(minSpaceX, static_cast<float>(spaceX / pow(2, dep - 1))), b_nodes[par].getPosition().y + 2 * RADIUS + spaceY - dep));
			}
...
void HeapMainState::initEdge()
{
	for (int i = 1; i < maxNodes; i++)
	{
		...
		if (i % 2 == 1) { // left child
			...
			edge.setStart(sf::Vector2f(center.x + cos(DEG_TO_RAD(30)) * RADIUS, center.y - sin(DEG_TO_RAD(30)) * RADIUS));
			
			//find center of parent
			center = b_nodes[heap.parent(i)].getPosition();
			edge.setEnd(sf::Vector2f(center.x - cos(DEG_TO_RAD(30)) * RADIUS, center.y + sin(DEG_TO_RAD(30)) * RADIUS));
		}
		else { // right child
			//find center of the node
			sf::Vector2f center = b_nodes[i].getPosition();
